/**
 * @file WheelleggedHWSim.cpp
 *
 * @brief This file defines the WheelleggedHWSim class, which is responsible for simulating a legged robot.
 *
 * © [2023] LimX Dynamics Technology Co., Ltd. All rights reserved.
 */

#include "wheellegged_gazebo/WheelleggedHWSim.h"
#include <gazebo_ros_control/gazebo_ros_control_plugin.h>

static limxsdk::WheelleggedSim *wl;

namespace wheellegged_gazebo
{
  bool WheelleggedHWSim::initSim(const std::string &robot_namespace, ros::NodeHandle model_nh, gazebo::physics::ModelPtr parent_model,
  ...
  void WheelleggedHWSim::readSim(ros::Time time, ros::Duration period)
  {
    ...
    if (joint_names_.size() == wl->getMotorNumber())
    {
      ...
      wl->publishImuDataForSim(imu_data);
    }
    else
    {
  ...
  void WheelleggedHWSim::writeSim(ros::Time time, ros::Duration period)
  {
  ...
  void WheelleggedHWSim::parseImu(XmlRpc::XmlRpcValue &imuDatas, const gazebo::physics::ModelPtr &parentModel)
  {
  ...
  void WheelleggedHWSim::parseContacts(XmlRpc::XmlRpcValue &contactNames)
  {
  ...
  int WheelleggedHWSim::parseJointIndex(const std::string &jointName)
  {
    int leg_index = 0;
    int joint_index = 0;
...
// Export the WheelleggedHWSim class as a plugin for gazebo_ros_control::RobotHWSim
PLUGINLIB_EXPORT_CLASS(wheellegged_gazebo::WheelleggedHWSim, gazebo_ros_control::RobotHWSim)
